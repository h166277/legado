...
import io.legado.app.help.readaloud.SpeechRoutePreviewPlayer
 ...
 * 业务逻辑见 [BookDubbingWorkspace]，朗读面板复用同一套。
 */
class BookDubbingActivity : BaseActivity<ViewBinding>(
    ...
    override fun onDestroy() {
        SpeechRoutePreviewPlayer.clear()
        workJob?.cancel()
        super.onDestroy()
    }
    ...
    private fun load() {
        ...
        lifecycleScope.launch {
            val data = BookDubbingWorkspace.load(bookUrl)
            book = data.book
            characterBookKey = data.characterBookKey
            roles = data.roles
    ...
    private fun analyzeRole(character: BookCharacter) {
        ...
        runWork("正在分析「${character.displayName()}」音色…") { _ ->
            val result = BookDubbingWorkspace.analyzeRole(b, character, overwrite = true)
    ...
    private fun analyzeAllRoles() {
        val b = book ?: return toastOnUi("书籍不存在")
        val targets = roles.map { it.character }
        if (targets.isEmpty()) {
            toastOnUi("没有可分析的角色卡")
            return
        }
        runWork("一键分析角色音色 0/${targets.size}") { updateBusy ->
            val done = BookDubbingWorkspace.analyzeAllRoles(b, targets) { d, total ->
                updateBusy("一键分析角色音色 $d/$total")
            }
    ...
    private fun previewRole(character: BookCharacter) {
        runWork("试听「${character.displayName()}」…") { _ ->
            val result = BookDubbingWorkspace.previewRole(character)
            toastOnUi(
                if (result.ok) {
                    "试听：${character.displayName()} · ${result.engineLabel.ifBlank { result.message }}"
                } else {
                    result.message.ifBlank { "试听失败" }
                }
            )
        }
    }
    ...
    private fun reassignRole(character: BookCharacter) {
        runWork("重新分配「${character.displayName()}」发言人…") { _ ->
            val entity = BookDubbingWorkspace.ensureCharacterEntity(book, character)
            val ok = BookDubbingWorkspace.reassignRole(entity)
    ...
    private fun assignAllVoices() {
        runWork("批量分配未配置发言人…") { _ ->
            val n = BookDubbingWorkspace.assignAllVoices(characterBookKey)
    ...
    private fun analyzeOneChapter(index: Int) {
        ...
        runWork("分析章节 #${index + 1}…") { _ ->
            val outcome = BookDubbingWorkspace.analyzeChapter(b, index, force = true)
            toastOnUi(
    ...
    private fun analyzeAllChapters() {
        ...
        runWork("全部 AI 分析…") { updateBusy ->
            val (ok, total) = BookDubbingWorkspace.analyzeAllChapters(
                book = b,
                concurrency = chapterConcurrency
            ) { done, all, okCount ->
                updateBusy("全部 AI 分析 $done/$all · 成功 $okCount")
            }
            if (total == 0) toastOnUi("没有章节")
            else toastOnUi("分析结束：成功 $ok / $total")
            load()
        }
    }
